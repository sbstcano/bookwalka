(function(){function e(e,t,n){let{innerWidthCss:r,innerHeightCss:i,cropCss:a}=e;if(r<=0||i<=0)throw Error(`Invalid viewport CSS dimensions`);if(t<=0||n<=0)throw Error(`Invalid screenshot dimensions`);let o=t/r,s=n/i;return{x:Math.round(a.left*o),y:Math.round(a.top*s),width:Math.round(a.width*o),height:Math.round(a.height*s)}}console.log(`Bookwalka: Content script injected successfully! (URL: `+window.location.href+`)`);function t(){let e=document.createElement(`div`);e.style.position=`fixed`,e.style.bottom=`20px`,e.style.left=`50%`,e.style.transform=`translateX(-50%)`,e.style.background=`rgba(239, 68, 68, 0.95)`,e.style.color=`#ffffff`,e.style.padding=`12px 24px`,e.style.borderRadius=`8px`,e.style.fontFamily=`-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif`,e.style.fontSize=`14px`,e.style.fontWeight=`600`,e.style.zIndex=`2147483647`,e.style.boxShadow=`0 4px 20px rgba(0, 0, 0, 0.4)`,e.style.border=`1px solid rgba(255, 255, 255, 0.2)`,e.style.textAlign=`center`,e.style.pointerEvents=`auto`,e.innerText=`Bookwalka: Extension was reloaded. Please refresh the page to continue.`,document.body.appendChild(e),setTimeout(()=>{e.style.transition=`opacity 0.5s ease`,e.style.opacity=`0`,setTimeout(()=>e.remove(),500)},5e3)}var n=!0;typeof chrome<`u`&&chrome.storage&&chrome.storage.local&&(chrome.storage.local.get([`extensionEnabled`],e=>{e.extensionEnabled!==void 0&&(n=!!e.extensionEnabled)}),chrome.storage.onChanged.addListener((e,t)=>{t===`local`&&e.extensionEnabled&&(n=!!e.extensionEnabled.newValue,n||d())}));var r=null,i=null,a=null,o=0,s=0,c=!1,l=null,u=null;function d(){r&&(r.remove(),r=null,l=null,u=null,document.removeEventListener(`keydown`,f))}function f(e){if(typeof chrome>`u`||!chrome.runtime||!chrome.runtime.id){document.removeEventListener(`keydown`,f);return}e.key===`Escape`&&(d(),chrome.runtime.sendMessage({type:`CANCEL_SELECTION`}))}function p(){if(typeof chrome>`u`||!chrome.runtime){t();return}if(!n)return;if(window!==window.top){chrome.runtime.sendMessage({type:`REQUEST_START_SELECTION`});return}d(),r=document.createElement(`div`),r.id=`bookwalker-translator-selection-host`,document.body.appendChild(r);let e=r.attachShadow({mode:`open`}),i=document.createElement(`style`);i.textContent=`
    .overlay {
      position: fixed;
      top: 0;
      left: 0;
